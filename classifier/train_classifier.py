...
from constants_classifier import NUM_LABELS
from labels import load_labeled_images
from recordings import load_data
from features_classifier import StateClassifier
from util_classifier import Augmentation, atomic_save, mirror_obs
...
def main():
    ...
    for i in itertools.count():
        ...
        if USE_MIXMATCH:
            ...
            difference=test_loss-loss.item()
            print('step %d: test=%f mixmatch=%f difference=%f' % (i, test_loss, loss.item(), difference))
            if difference > 0.1:
                if len(idx_list) > 0 and idx_list[len(idx_list)-1] == i-1:
                    idx_list.append(i)
                    if len(idx_list) > overfit_num:
                        print("Stop at step %d" % i)
                        break
                elif len(idx_list) == 0:
                    idx_list.append(i)
